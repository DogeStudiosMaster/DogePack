#version 150
#moj_import <betalight.glsl>

layout(std140) uniform LightmapInfo {
    float AmbientLightFactor;
    float SkyFactor;
    float BlockFactor;
    float NightVisionFactor;
    float DarknessScale;
    float DarkenWorldFactor;
    float BrightnessFactor;
    vec3 SkyLightColor;
    vec3 AmbientColor;
} lightmapInfo;

in vec2 texCoord;
out vec4 fragColor;

int spread(float f, int x) {
    return clamp(int(floor(f * float(x + 1))), 0, x);
}

void main() {
    int block_light = spread(texCoord.x, 15);
    int sky_light = spread(texCoord.y, 15);
    int sky_factor = clamp(spread(1.0 - lightmapInfo.SkyFactor, 15), 0, 11);

    int adjusted_sky = clamp(sky_light - sky_factor, 0, 15);

    // Convert both light values to vec3 for proper max comparison
    vec3 SkyLight = BETA_LIGHT[adjusted_sky] * lightmapInfo.SkyLightColor;
    vec3 BlockLight = vec3(BETA_LIGHT[block_light]);

    // Use component-wise maximum
    vec3 combinedLight = max(SkyLight, BlockLight);
    vec3 color = combinedLight / 255.0;

    // Apply ambient lighting
    color = mix(color, lightmapInfo.AmbientColor, clamp(lightmapInfo.AmbientLightFactor / 1.9, 0.0, 1.0));

    // Night Vision logic
    if (lightmapInfo.NightVisionFactor > 0.0) {
        float max_component = max(color.r, max(color.g, color.b));
        if (max_component < 1.0) {
            vec3 bright_color = color / max_component;
            color = mix(color, bright_color, lightmapInfo.NightVisionFactor);
        }
    }

    color = clamp(color, 0.0, 1.0);

    // Apply brightness correction using gamma-like curve
    fragColor = pow(vec4(color, 1.0), vec4(1.0 / (1.0 + lightmapInfo.BrightnessFactor)));
}
